...
# Clave de Zapper (debe estar en .env como ZAPPER_API_KEY=tu_api_key)
...
# ─── Zapper GraphQL helper ──────────────────────────────────────────────────────

def fetch_net_worth_zapper(address: str) -> float:
    """
    Llama al endpoint GraphQL de Zapper para obtener el net worth (USD) de una dirección.
    Retorna el valor total en USD como float. Lanza excepción si hay error HTTP o parseo.
    """
    ...
    try:
        ...
        nfts_usd     = portfolio["nftBalances"]["totalBalanceUSD"] or 0.0
        net_worth    = tokens_usd + apps_usd + nfts_usd
        return float(net_worth)
    except (KeyError, TypeError) as e:
        raise RuntimeError(f"No se pudo parsear respuesta de Zapper: {e}\nRespuesta completa: {data}")
...
if __name__ == "__main__":
    # 1) Obtener balances desde Zapper (net worth en USD) para cada (address, fundid)
    ...
    for address, fundid in URL_FUNDID_LIST:
        try:
            ...
            print(f"🔍 Zapper[{address}] = ${net_worth:,.2f} (a las {ts})")
            results.append((fundid, net_worth, ts))
        except Exception as e:
            ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"❌ Falló la consulta a Zapper[{address}]: {e}")
    ...
    # 3) Escribo todos los resultados de Zapper en RAW y actualizo tokens de usuarios
