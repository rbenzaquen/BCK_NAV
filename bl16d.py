...
def fetch_net_worth_debank(address: str) -> float:
    url = f"{DEBANK_BASE}/v1/user/total_balance"
    try:
        resp = requests.get(
            url,
            params={"id": address},
        ...
        raise RuntimeError(f"Error de red al consultar DeBank: {e}") from e

    if resp.status_code != 200:
        raise RuntimeError(f"HTTP {resp.status_code} de DeBank: {resp.text}")

    data = resp.json() or {}
    total = data.get("total_usd_value")
    if total is None:
        raise RuntimeError(f"Respuesta inesperada de DeBank: {data}")
    return float(total)
